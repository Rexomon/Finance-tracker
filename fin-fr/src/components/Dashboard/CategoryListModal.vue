        ...
        <div class="mb-4">
          <div class="flex items-center space-x-4">
            <button @click="filterType = 'all'"
              ...
              class="px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-500 hover:text-white transition-colors">
              All Categories
            </button>
            <button @click="filterType = 'income'"
              :class="filterType === 'income' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'"
              class="px-4 py-2 rounded-md text-sm font-medium hover:bg-green-500 hover:text-white transition-colors">
              Income
            </button>
            <button @click="filterType = 'expense'"
              :class="filterType === 'expense' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700'"
              class="px-4 py-2 rounded-md text-sm font-medium hover:bg-red-500 hover:text-white transition-colors">
              Expense
            </button>
          </div>
        </div>
              ...
              <tr v-for="category in filteredCategories" :key="category._id">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div v-if="editingCategory === category._id">
                    <input v-model="editForm.categoryName" type="text" maxlength="50" pattern="[a-zA-Z0-9\s\-_]+"
                      ...
                      @keyup.enter="saveCategory(category._id)" @keyup.esc="cancelEdit" />
                  </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                  <div v-if="editingCategory === category._id" class="flex space-x-2">
                    <button @click="saveCategory(category._id)" :disabled="loading"
                  ...
                  <div v-else class="flex space-x-2">
                    <button @click="startEdit(category)" class="text-blue-600 hover:text-blue-900">
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button @click="deleteCategory(category._id)" :disabled="loading"
